...
APP_VERSION = '0.1.1alpha-20201017'
...
def convert2php(code):
    if code != None:
        ...
        subs = [
          [r'\$\{\{\{([^#]+)\}\}\}', r'" . (\1) . "'],
          [r'\$\{([^\}]+)\}', r'" . (\1) . "'],
          ['\\{([\\w\\d]+)\\.([\\w\\d]+)\\.Split\\(\'([^\']+)\'\\)\\[(\\d+)\\]\\}', '" . explode(\'\\3\', $\\1[\'\\2\'])[\\4] . "'],
          ['\\{([\\w\\d]+)\\.([\\w\\d]+)\\.Split\\(\'([^\']+)\'\\)\\[(\\d+)\\]\\.Split\\(\'([^\']+)\'\\)\\[(\\d+)\\]\\}', '" . explode(\'\\5\', explode(\'\\3\', $\\1[\'\\2\'])[\\4])[\\6] . "'],
          ['\\{IIF\\(([\\w\\d]+)\\.([\\w\\d]+)\\.Contains\\(\\\\"([^\\\\]+)\\\\"\\) *, *\\\\"([^\\\\]+)\\\\" *, *\\\\"([^\\\\]+)\\\\"\\)\\}', '" . ($\\1[\'\\2\'] == \'\\3\' ? \'\\4\' : \'\\5\') . "'],
          ['\\{IIF\\(([\\w\\d]+)\\.([\\w\\d]+)\\.Contains\\(\\\\"([^\\\\]+)\\\\"\\) *, *([^,]+), *([^\\)]+)\\)\\}', '" . ($\\1[\'\\2\'] == \'\\3\' ? \\4 : \\5) . "'],
          ['\\{IIF\\(([\\w\\d]+)\\.([\\w\\d]+)\\.Contains\\("([^"]+)"\\) *, *([^,]+), *([^\\)]+)\\)\\}', '" . ($\\1[\'\\2\'] == \'\\3\' ? \\4 : \\5) . "'],
          ['\\{([\\w\\d]+)\\.([\\w\\d]+)\\.Split\\(\'([^\']+)\'\\)\\[(\\d+)\\]\\.Substring\\( *(\\d+) *, *(\\d+) *\\)\\}', '" . substr(explode(\'\\3\', $\\1[\'\\2\'])[\\4], \\5, \\6) . "'],
          ['\\{([\\w\\d]+)\\.([\\w\\d]+)\\[(\\d+)\\]\\}', '{$\\1[\'\\2\'][\\3]}'],
          ['\\{([\\w\\d]+)\\.([\\w\\d]+)\\}', '{$\\1[\'\\2\']}']
        ]
        ...
        converted = code
        for sub in subs:
            converted = re.sub(sub[0], sub[1], converted, flags=re.MULTILINE)
...
halign = {'Center':'C', 'Justify':'J', 'Left':'L', 'Right':'R', 'Width':'J'}
...
if 'Culture' in stijson and stijson['Culture'] == 'fa-IR':
    lang_pack = '''
...
$pdf->AddPage();''' % 
    (APP_VERSION,
     ...
     stijson['Pages']['0']['Margins']['Right'], stijson['Pages']['0']['Margins']['Bottom'], lang_pack,
...
for components in sticomponents:
    if sticomponents[components]['Ident'] == 'StiText':
        ...
        if 'Text' in sticomponents[components]:
            ...
            # print('// ' + Text)
            #if re.match('.*{([^}]+)}.*', Text, flags=re.MULTILINE) != None:
        ...
        if 'ImageBytes' in sticomponents[components] and len(sticomponents[components]['ImageBytes']) > 100: # constraint
            imgB64, imgType = getIImage(sticomponents[components]['ImageBytes'])
            imgCode = '''$pdf->Image('@' . base64_decode('%s'), %.02f, %.02f, %.02f, %.02f, '%s', '', '', true, 150, '', false, false, 0, false, false, false);''' % (imgB64, rect['X'], rect['Y'], rect['Width'], rect['Height'], imgType)
        if 'DataColumn' in sticomponents[components] and len(sticomponents[components]['DataColumn']) > 1: # datacolumn! (variable)
